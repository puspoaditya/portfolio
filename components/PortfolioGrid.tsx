...
import { projectsId, categoriesId } from "@/lib/portfolio-id";
...
export function PortfolioGrid() {
  ...
  const t = getT(lang);
  const allProjects = lang === "id" ? projectsId : projects;
  const allCategories = lang === "id" ? categoriesId : categories;
    ...
    <section id="portfolio" className="container-x py-20">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {([allLabel, ...allCategories] as const).map((c) => (
            <button
              key={c}
              onClick={() => setFilter(c === allLabel ? "All" : c)}
              className={`text-xs px-3 py-1.5 rounded-full border transition ${
                (c === allLabel && filter === "All") || filter === c
